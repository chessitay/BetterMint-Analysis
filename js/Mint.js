...
class StockfishEngine {
  ...
  ProcessMessage(event) {
    ...
    if (line === "uciok") {
      this.loaded = true;
      console.log("Stockfish engine UCI initialization complete");
      ...
      // Verify that the engine is truly ready with a test position
      this.send("position fen rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");
      this.send("go depth 1");
    } else if (line === "Load eval file success: 1") {
      console.log("Stockfish evaluation file loaded successfully");
      this.isEvaluating = false;
      this.isRequestedStop = false;
      ...
      if (depthMatch && scoreMatch && pvMatch) {
        ...
        console.log("Stockfish analysis complete, best move found");
        this.isEvaluating = false;
        if (this.goDoneCallbacks.length > 0) {
          let copy = this.goDoneCallbacks;
          this.goDoneCallbacks = [];
...
class BetterMint {
  ...
  onEngineLoaded() {
    if (window.toaster) {
      window.toaster.add({
        id: "bettermint-loading",
        duration: 3000,
        icon: "circle-info",
        content: `BetterMint V2 is initializing...`,
      });
      
      // Show a message asking users to wait for full loading
      setTimeout(() => {
        window.toaster.add({
          id: "bettermint-wait",
          duration: 10000,
          icon: "circle-info",
          content: `Please wait 10 seconds for all components to fully load. BetterMint is preparing analysis tools.`,
          style: {
            position: "fixed",
            bottom: "60px",
            right: "30px",
      ...
      // Final confirmation message after 10 seconds
      setTimeout(() => {
        window.toaster.add({
          id: "bettermint-ready",
          duration: 4000,
          icon: "circle-checkmark",
          content: `BetterMint V2 is now fully loaded and ready to use!`,
          style: {
            position: "fixed",
            bottom: "60px",
            right: "30px",
  ...
  verifySystemStatus() {
    ...
    const statusChecks = {
      engine: this.engine && this.engine.loaded && this.engine.ready,
      game: this.game && this.game.controller,
      evalBar: !getValueConfig(enumOptions.EvaluationBar) || (this.game && this.game.evalBar !== null),
      depthBar: !getValueConfig(enumOptions.DepthBar) || (this.game && this.game.depthBar !== null),
      ecoTable: eTable !== null
    };
    
    const allSystemsGo = Object.values(statusChecks).every(status => status === true);
    
    if (allSystemsGo) {
      if (window.toaster) {
        window.toaster.add({
          id: "bettermint-all-systems-go",
          duration: 4000,
          icon: "circle-checkmark",
          content: `BetterMint V2 fully operational! All systems verified.`,
          style: {
            position: "fixed",
            bottom: "60px",
            right: "30px",
      ...
      // Report issues
      const issues = Object.entries(statusChecks)
        .filter(([_, value]) => value === false)
        .map(([key, _]) => key)
        .join(", ");
        
      console.warn(`BetterMint V2 status issues: ${issues}`);
      
      if (window.toaster) {
        window.toaster.add({
          id: "bettermint-status-issues",
          duration: 5000,
          icon: "circle-warning",
          content: `BetterMint V2 issues detected: ${issues}. Attempting recovery...`,
          style: {
            position: "fixed",
            bottom: "60px",
            right: "30px",
        ...
        // Try to recover
        if (!statusChecks.engine && this.engine) {
          this.engine.restartGame();
        }
        
        if ((!statusChecks.evalBar || !statusChecks.depthBar) && this.game) {
          this.game.CreateAnalysisTools();
        }
      }
    }
  }
...
function InitBetterMint(chessboard) {
  ...
  if (window.toaster) {
    window.toaster.add({
      id: "bettermint-loading-init",
      duration: 10000,
      icon: "circle-info",
      content: `Please wait 10 seconds for BetterMint to fully load...`,
      style: {
        position: "fixed",
        bottom: "60px",
        right: "30px",
    ...
    try {
      ...
      if (window.toaster) {
        window.toaster.add({
          ...
          duration: 5000,
          icon: "circle-error",
          content: `BetterMint error: Could not initialize`,
          style: {
            position: "fixed",
            bottom: "60px",
            right: "30px",
